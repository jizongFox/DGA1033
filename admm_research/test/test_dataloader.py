...
def show_img_mask_weakmask(img,gt,weak):
    ...
    plt.contourf(gt.squeeze()[x_min-margin:x_max+margin, y_min-margin:y_max+margin],colors ='red',levels=[0.5,0.8])

    y = np.linspace(0, x_max-x_min+2*margin, x_max-x_min+2*margin)
    x = np.linspace(0, y_max-y_min+2*margin, y_max-y_min+2*margin)
    X, Y = np.meshgrid(x, y)
    plt.contourf(X,Y,weak.squeeze()[x_min-margin:x_max+margin, y_min-margin:y_max+margin],4, cmap =my_cmap,levels=1)
    plt.tight_layout()
    plt.axis('off')
    plt.show()
    plt.pause(0.1)
