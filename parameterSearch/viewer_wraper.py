...
def main(args: argparse.Namespace) -> str:
    ...
    sorted_record = record.sort_values(by=['fd'], ascending=False).head(args.n_columns) if args.lamda_all else record[record['lamda']>0].sort_values(by=['fd'], ascending=False).head(args.n_columns)
